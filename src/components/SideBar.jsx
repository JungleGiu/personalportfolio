    ...
    <aside className='w-1/5 min-h-screen flex flex-col  border-r-2 border-blue-950 dark:border-gray-200'>
        ...
        <ul className='px-5 pt-5'>
            {categories.map((category) => (
                <li className='group cursor-pointer hover:text-gray-500 dark:hover:text-gray-200' onClick={() => setCategory(category)} key={category}><span className='group-hover:animate-blink animate:none'>_</span>{category}</li>
            ))}
        </ul>
        </div>
    </aside>
